...
aarticle_features = pd.DataFrame({
...
def create_customer_features(selected_customers_df):
    ...
    gold_articles = aarticle_features[aarticle_features['Gold'] == 1]['Article'].tolist()
    ...
    silver_articles = aarticle_features[aarticle_features['Silver'] == 1]['Article'].tolist()
    ...
    heritage_articles = aarticle_features[aarticle_features['Heritage'] == 1]['Article'].tolist()
...
def score_and_sort(article_number: str, excluded_articles: list):
    """
    Compute ML + item-based CF scores and return a sorted dataframe by Score (desc).
    Excludes customers who have already purchased the selected article AND
    ANY of the user-selected 'excluded_articles'.
    """
    X = customer_article_matrix.values  # (n_customers, n_articles)
    article_index = int(article_number.split("_")[1])

    # --- Exclude customers who already purchased the selected article (use > 0, not == 1)
    already_purchased_selected = customer_article_matrix.iloc[:, article_index] > 0

    # --- Exclude customers who bought ANY of the user-selected exclusion articles
    exclude_cols = [col for col in customer_article_matrix.columns if col in excluded_articles]
    if exclude_cols:
        purchased_any_excluded = (customer_article_matrix[exclude_cols] > 0).any(axis=1)
    else:
        purchased_any_excluded = pd.Series(False, index=customer_article_matrix.index)
    ...
    st.session_state.excluded_prev_articles = int(purchased_any_excluded.sum())
    st.session_state.excluded_customers = int(combined_exclude.sum())  # total unique excluded
    st.session_state.eligible_customers = int((~combined_exclude).sum())

    # Filter data to only eligible customers
    X_eligible = X[~combined_exclude]
    customer_data_eligible = customer_data[~combined_exclude].reset_index(drop=True)

    if len(X_eligible) == 0:
        # No one left after exclusions
        return pd.DataFrame()
    ...
    ml_scores = model.predict_proba(X_eligible)[:, 1] if len(model.classes_) > 1 else np.zeros(X_eligible.shape[0])
...
article_number = st.selectbox(
    "Select Article Number", aarticle_features["Article"]
)

# NEW — allow excluding customers who bought any of these articles before
...
# NEW — track breakdown and last exclusions
if "excluded_selected_article" not in st.session_state:
    st.session_state.excluded_selected_article = 0
if "excluded_prev_articles" not in st.session_state:
    st.session_state.excluded_prev_articles = 0
if "last_excluded_articles" not in st.session_state:
    st.session_state.last_excluded_articles = tuple()

# --- Trigger scoring and persist results ---
if st.button("Score Customers"):
    with st.spinner("Scoring customers..."):
        st.session_state.df_sorted = score_and_sort(article_number, excluded_articles)
        st.session_state.last_scored_article = article_number
        st.session_state.last_excluded_articles = tuple(sorted(excluded_articles))
...
if st.session_state.df_sorted is not None:
    # Warn user if inputs changed after scoring (article or exclusions)
    current_excl = tuple(sorted(excluded_articles))
    if (article_number != st.session_state.last_scored_article) or \
       (current_excl != st.session_state.get("last_excluded_articles", tuple())):
        st.info("Inputs changed. Click **Score Customers** to refresh rankings for the new settings.")
    else:
        # Only show exclusion information if we're viewing results for the current inputs
        if st.session_state.total_customers > 0:
            st.info(
                f"**Customer Filtering:** "
                f"{st.session_state.excluded_customers} excluded in total — "
                f"{st.session_state.excluded_selected_article} (already purchased {st.session_state.last_scored_article}) + "
                f"{st.session_state.excluded_prev_articles} (previously purchased from your exclusion list). "
                f"{st.session_state.eligible_customers} eligible customers remain."
            )

    # Check if no eligible customers for current inputs
    if (article_number == st.session_state.last_scored_article) and \
       (current_excl == st.session_state.get("last_excluded_articles", tuple())) and \
       (len(st.session_state.df_sorted) == 0):
        st.warning("⚠️ No customers remain after applying the current exclusions. Adjust the exclusion list and try again.")
        st.stop()
